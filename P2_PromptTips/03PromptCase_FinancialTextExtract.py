...
# 1. get client obj, openai object
client = OpenAI(
...
# 2. send questions to the model
total_token = 0
for q in questions:
    chat = client.chat.completions.create(
        model="qwen3.6-plus",
        messages=messages + [{"role": "user", "content": f"按照示例，现在抽取这个句子的信息：{q}"}],
        # stream=True
    )

    total_token += chat.usage.completion_tokens
    print(chat.choices[0].message.content)

print("\n"+"completion_tokens: "+str(total_token))